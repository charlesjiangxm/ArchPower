#!/usr/bin/env bash
# Run the Ridge × FeatureSelector sweep on c906-db.
#
# Iterates over 6 selectors (sequential and from_model are excluded), trains
# RidgeCV on the selected columns for each, captures wall time and a stdout
# log per method, then aggregates the per-method output directories into a
# single sweep folder with a generated SUMMARY.md.
#
# Usage (from anywhere):
#   ./run_ridge_sweep.sh [--split SPLIT] [--presim PRESIM_SUBDIR]
#                        [--sweep-dir SWEEP_DIR]
...
#   --sweep-dir    ridge_c906_sweep_feature_selection_presim
#
...
# --extra           appended verbatim to every c906_ridge.py invocation
#                   (useful for e.g. --top_k 500, --ridge_alphas "1,10,100")
...
SPLIT="time_ordered"
PRESIM="presim_large"
SWEEP_DIR="ridge_c906_sweep_feature_selection_presim"
SKIP_RUN=0
SKIP_AGG=0
EXTRA_ARGS=""
...
RUNNER="$SCRIPT_DIR/c906_ridge.py"
...
LOG_DIR="$OUTPUT_DIR/_ridge_sweep_logs"
mkdir -p "$LOG_DIR"

cd "$REPO_ROOT"

echo "============================================================"
echo "  Ridge × FeatureSelector sweep"
echo "  split=$SPLIT  presim=$PRESIM"
echo "  sweep-dir=$SWEEP_DIR"
[[ -n "$EXTRA_ARGS" ]] && echo "  extra=$EXTRA_ARGS"
echo "  runner=$RUNNER"
echo "============================================================"

# Returns the output directory c906_ridge.py uses for a given method.
# Logic must mirror c906_ridge.py:main().
out_dir_for() {
  local method="$1"
  local suffix=""
  [[ "$method" != "pearson" ]] && suffix+="_$method"
  [[ "$PRESIM" != "presim" ]]  && suffix+="_$PRESIM"
  echo "$OUTPUT_DIR/ridge_c906_${SPLIT}${suffix}"
}
...
out = []
out.append(f"# Ridge on c906 — feature-selection sweep")
out.append("")
out.append(f"Sweep settings: `--split {split}` `--presim_subdir {presim}`, "
           f"RidgeCV defaults from c906_ridge.py. "
